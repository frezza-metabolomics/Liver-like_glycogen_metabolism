...
tTest <- function(Result_Folder ,
  ...
  if("denominator" %in% names(DMA_SettingsInfo)==FALSE  & "numerator" %in% names(DMA_SettingsInfo) ==FALSE){
    ...
    denominator <-unique(DMA_SettingsFile$Conditions)[1]
    numerator <-unique(DMA_SettingsFile$Conditions)[2]
    comparisons <- combn(unique(conditions), 2) %>% as.matrix()
    #Settings:
  }
  comparisons <- data.frame(c(numerator, denominator))
  if (Paired == TRUE  & is.null(PairedData) == TRUE) {
    stop("Check input. There is no column defined to identify the paired samples" )
  ...
  ## 3. ------------ Check outputs ----------- ##
  if (is.null(OutputName)){ OutputName <- "tTestRes.Rdata"
  }else{ OutputName <- paste("tTest_" ,OutputName, ".Rdata" ,sep = "" )}
  if (file.exists(file.path(RDS_folder,  OutputName))==TRUE & ForceRun == FALSE){
    ...
    if (Paired == F){
      ...
      T_C1vC2 <-mapply(t.test, x= as.data.frame(C2), y = as.data.frame(C1), SIMPLIFY = F, var.equal = variance_equality)
    }else{
      message(paste("Paired"," t.test " , "statistics applied"))
      Data_DMA_Cond1 <- DMA_SettingsFile %>% dplyr::filter(Conditions %in% numerator) 
      message(paste("Paired","for condition one")," ",names(Data_DMA_Cond1)," ", row.names(Data_DMA_Cond1))
      Data_DMA_Cond2 <- DMA_SettingsFile %>% dplyr::filter(Conditions %in% denominator)
      message(paste("Paired","for condition one")," ",names(Data_DMA_Cond2)," ", row.names(Data_DMA_Cond2))
      if (dim(Data_DMA_Cond2)[1] != dim(Data_DMA_Cond1)[1]) {warning("numerator and denominotor are not equal for the pairwise t-test to take place")} 
      #list(C1 = Data_DMA_Cond1$Biological_Replicates, C2 = Data_DMA_Cond2$Biological_Replicates)
      # Data_DMA_Cond1[c(45:49),]
      # Data_DMA_Cond2[c(45:49),]
      sortedC1 <- C1 %>% arrange(factor(row.names(C1), levels = unique(Data_DMA_Cond1[PairedData])))
      sortedC2 <- C2 %>% arrange(factor(row.names(C2), levels = unique(Data_DMA_Cond2[PairedData])))
      
      T_C1vC2 <- base::mapply(t.test, x= as.data.frame(sortedC2), y = as.data.frame(sortedC1), paired = T, SIMPLIFY = F, var.equal = variance_equality)    
    }
    ...
    if (Save_as_Results == "xlsx"){
      xlsDMA <- file.path(Result_Folder,paste0("DMA_Output_",toString(numerator),"_vs_",toString(denominator), ".xlsx"))   # Save the DMA results table
      writexl::write_xlsx(Output,xlsDMA, col_names = TRUE) # save the DMA result DF
    }else if (Save_as_Results == "csv"){
      csvDMA <- file.path(Result_Folder,paste0("DMA_Output_",toString(numerator),"_vs_",toString(denominator), ".csv"))
      write.csv(Output,csvDMA) # save the DMA result DF
    }else if (Save_as_Results == "txt"){
      txtDMA <- file.path(Result_Folder,paste0("DMA_Output_",toString(numerator),"_vs_",toString(denominator), ".txt"))
    ...
    if (CoRe== TRUE){
      ...
      colnames(DMA_Output)[grep("log2", tolower(colnames(DMA_Output)))] <- "Log2FC"
    }
    ...
    VolcanoPlot <- invisible(Volcano(Plot_Settings="Standard",
                                     ...
                                     SelectLab= Labels,
                                     Connectors=  TRUE,
                                     Subtitle=  bquote(italic("Differential Metabolite Analysis, t-test")),
                                     Theme= NULL,
                                     Save_as_Plot= NULL))
    ...
    volcanoDMA <- file.path(Result_Folder,paste0( "Volcano_Plot_",toString(numerator),"_versus_",toString(denominator),".svg"))
    ...
    # Make Result list
    suppressWarnings(tTestRes <- list("DF" = list("DMA_result"=DMA_Output),"Plot"=list("Volcano"=volplotList)))
    # Save output RDS
    save(tTestRes, file = file.path(RDS_folder,  OutputName))
  }
  return(tTestRes)
}
