...
##
## Purpose of script: Data Visualisation of the Viz analysis to aid biological interpretation
##
...
## ---------------------------
#'
#' This script allows you to perform PCA plot visualization using the results of the Viz analysis
...
PCA <- function(Plot_SettingsInfo= NULL,
  ...
  ## ------------ Setup and installs ----------- ##
  RequiredPackages <- c("tidyverse","ggfortify", "ggplot2", "patchwork")
  ...
  if (!is.null(Save_as_Plot)) {
    ...
    #rm(ProjectFolder)
    #rm(ProjectCode)
    #rm(Analysed_data_folder)
    if(!exists("x")){
      ProjectFolder <- getwd()
      ProjectCode <- NULL
    } else {ProjectCode <- strsplit(ProjectFolder, "_")[[1]][[ length(strsplit(ProjectFolder, "_")[[1]])]] }
    if(is.null(FolderName)){name <- paste("Results",sep = "" ) }else{ name <- paste("Results",FolderName,sep = "_" )}
    if (is.null(ProjectCode)){Analysed_data_folder <- file.path(ProjectFolder,Sys.Date()) } else {
      Analysed_data_folder <- file.path(ProjectFolder,paste0("5.", ProjectCode, "Analysed Data", sep = " ")) }
    ...
    if (!dir.exists(Results_folder)) {dir.create(Results_folder, recursive = T)} # Make Results folder
    Results_folder_plots_PCA_folder = file.path(Results_folder, "PCA")
  ...
  if(is.null(Plot_SettingsFile)==FALSE){
    InputPCA  <- merge(x=Plot_SettingsFile%>%rownames_to_column("UniqueID") , y=Input_data%>%rownames_to_column("UniqueID"), by="UniqueID", all.y=TRUE, sort = FALSE)%>%
  ...
  library(ggplot2)
  library(gridExtra)  # Or use library(patchwork)
  
  # Initialize an empty list to store the plots
  plots <- list()
  
  # Loop through all combinations of x and y from 1 to 5
  for (i in 1:5) {
    for (j in 1:5) {
      if (i != j) {  # Skip the cases where x == y
        pca_plot <- autoplot(prcomp(as.matrix(Input_data_m), scale. = as.logical(Scaling)),
                             x = i, y = j,
                             data = InputPCA,
                             colour = Param_Col,
                             fill = Param_Col,
                             ...
                             size = 3,
                             alpha = 0.8,
                             loadings = as.logical(Show_Loadings),
          ...
          ggtitle(paste(OutputPlotName, "PC", i, "vs PC", j)) +
        ...
        pca_plot <-ggplot() +  theme_void() + ggtitle(paste0("PC",i," vs ", "PC", j))
  ...
  # Combine all the plots in a grid layout
  ...
  if (!is.null(Save_as_Plot)) {
    if(OutputPlotName ==""){
      ggsave(file=paste(Results_folder_plots_PCA_folder,"/", "PCA", OutputPlotName, ".",Save_as_Plot, sep=""), plot=PCA, width=20, height=20, unit="cm")
    }else{
      ggsave(file=paste(Results_folder_plots_PCA_folder,"/", "PCA_", OutputPlotName, ".",Save_as_Plot, sep=""), plot=PCA, width=20, height=20, unit="cm")
    }
  }
