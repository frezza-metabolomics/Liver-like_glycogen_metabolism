...
Volcano <- function(Plot_Settings="Standard",
  ...
  if (!is.null(Save_as_Plot)) {
    if(is.null(FolderName)){
      name <- paste("Results",Sys.Date(),sep = "_" )
    }else{
      if(grepl('[^[:alnum:]]', FolderName)){
        stop("The 'FolderName' must not contain any special character.")
      }else{
        name <- paste("Results",Sys.Date(),FolderName,sep = "_" )
      }
    }
    WorkD <- getwd()
  ...
  if(Plot_Settings=="Standard"){############################################################################################################
    if("individual" %in% names(Plot_SettingsInfo)==TRUE){
      ...
      for (i in IndividualPlots){
        ...
        InputVolcano  <- merge(x=Plot_SettingsFile_Select,y=Input_data, by="Metabolite", all.x=TRUE, sort = FALSE)%>%
          na.omit()
        ...
        if(nrow(InputVolcano)>=1){
          ...
          Plot<- EnhancedVolcano::EnhancedVolcano(InputVolcano,
                                                  ...
                                                  pointSize = c(ifelse(SelectLab, 3, 1)),
                                                  labSize = 3,
                                                  axisLabSize = 10,
                                                  titleLabSize = 12,
                                                  subtitleLabSize = 11,
      ...
      if(is.null(Plot_SettingsFile)==FALSE){
        InputVolcano  <- merge(x=Plot_SettingsFile,y=Input_data, by="Metabolite", all.x=TRUE, sort = FALSE)%>%
          na.omit()
      }else{
      ...
      if(nrow(InputVolcano)>=1){
        ...
        # #Set the total heights and widths
        # Plot_Sized <-  plotGrob_Volcano(Input=Plot, keyvals = keyvals, keyvalsshape = keyvalsshape, OutputPlotName = OutputPlotName, Subtitle = Subtitle)
        # Plot <-Plot_Sized[[3]]
        # Plot <- ggplot2::ggplot() +
        #   annotation_custom(Plot)
        # 
    ...
    if("individual" %in% names(Plot_SettingsInfo)==TRUE){
      ...
      for (i in IndividualPlots){
        Plot_SettingsFile_Select <- subset(Plot_SettingsFile, individual == paste(i))
        InputVolcano  <- merge(x=Plot_SettingsFile_Select,y=Input_Comparison, by="Metabolite", all.x=TRUE, sort = FALSE)%>%
          na.omit()
        ...
        if(nrow(InputVolcano)>=1){
          ...
          #Set the total heights and widths
          PlotTitle <- paste(OutputPlotName, ": ", i, sep="")
          # Plot_Sized <-  plotGrob_Volcano(Input=Plot, keyvals = keyvals, keyvalsshape = keyvalsshape, OutputPlotName = PlotTitle, Subtitle = Subtitle)
          # Plot <-Plot_Sized[[3]]
          # Plot <- ggplot2::ggplot() +
          #   annotation_custom(Plot)
          # 
      ...
      if(is.null(Plot_SettingsFile)==FALSE){
        InputVolcano  <- merge(x=Plot_SettingsFile,y=Input_Comparison, by="Metabolite", all.x=TRUE, sort = FALSE)%>%
          na.omit()
      }else{
