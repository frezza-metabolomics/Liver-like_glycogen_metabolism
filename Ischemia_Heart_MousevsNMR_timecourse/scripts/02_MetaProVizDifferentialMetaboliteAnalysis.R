...
MetaProVizDMA <-function(Input_data =Data,
                         ...
                         STAT_pval = NULL, #STAT_pval ="t-test", # or wilcox-test if for some reason it is needed to be set differently
                         Paired = F,
                         padjVALs =   c("0.01","0.05","0.1"),
                         log2FCs = c("0.5","1","2"),
                         Input_Pathways = NULL,
                         OutputName= '',
                         CoRe=FALSE,
                         CoRe_data =  NULL,
                         removeMeta = c("valine-d8","hippurate-d5"),
  ...
  Experimental_design <- Experimental_design[Conditions]
  names(Experimental_design) <- "Conditions"
  
  # This is when we use the data_processed_summed data
  if ('Conditions' %in% colnames(Input_data)){
    # Cond <- Input_data$Conditions
    Input_data<- dplyr::select_if(Input_data, is.numeric)%>%
      select(-one_of(c("Analytical_Replicates", "Biological_Replicates", removeMeta)))
    ...
    Experimental_design <- Experimental_design %>%
      dplyr::filter(rownames(Experimental_design) %in% rownames(Input_data))
  }
  #library(dplyr)
  # select samples according to the input conditions
  C1 <- Experimental_design %>% dplyr::filter(Experimental_design$Conditions %in% Condition1) %>% rownames()
  C2 <- Experimental_design %>% dplyr::filter(Experimental_design$Conditions %in% Condition2) %>% rownames()
  
  Data_DMA_Cond1 <- Input_data %>% dplyr::filter(rownames(Input_data) %in% C1)
  Data_DMA_Cond2 <- Input_data %>% dplyr::filter( rownames(Input_data) %in% C2)
  
  if(CoRe==TRUE){
    if(Pooled==TRUE){
      print("Code missing")
    }else{
      Data_CoRe_Cond1 <- CoRe_data[Experimental_design$Conditions %in% Condition1,]
      Data_CoRe_Cond2 <- CoRe_data[Experimental_design$Conditions %in% Condition2,]
    }
  }
  
  Log2FC_Condition1 <- Input_data %>% dplyr::filter(rownames(Input_data) %in% C1) %>%  summarise_all("mean")
  Log2FC_Condition2 <- Input_data %>% dplyr::filter(rownames(Input_data) %in% C2) %>%  summarise_all("mean")
  ...
  # Select data
  shaptest <-  Input_data %>%
    dplyr::filter(Experimental_design$Conditions %in% Condition1 | Experimental_design$Conditions %in% Condition2)
  ...
  if (is.null(STAT_pval)){
    ...
    message("wilcox-test used")
    STAT_name = "wilcox-test"
    # if(STAT_pval =="t-test"){
  ...
  }else{  # select the test for hypothesis testing
    message("user selected a defined statistical test")
    if(STAT_pval=="t-test"){
      STAT_name = "t-test"
      STAT_pval= t.test
      if (!(sum(shapirotres$p.value > 0.05) > sum(shapirotres$p.value < 0.05))) {
        message("The data are not normally distributed. However, 't.test' is selected for Hypothesis testing. If samples per replicate are more than 5, please consider change it to 'wilcox-test'.")
      }else{
		message("The data are normally distributed. You have selected the correct statistical method, a 't.test'.")
	  }
      ...
      STAT_name = "wilcox-test"
      if ((sum(shapirotres$p.value > 0.05) > sum(shapirotres$p.value < 0.05))) {
        message("The data are normally distributed. However, 'wilcox.test' is selected for Hypothesis testing.Please consider change it to 't-test'.")
      }else{
		message("The data are not normally distributed. You have selected the correct statistical method, a wilcox.test.")
	  }
      if(dim(Data_DMA_Cond1)[1]<5 | dim(Data_DMA_Cond2)[1]<5){
          warning(paste0( "Very small number of samples for using wilcox-test. Consider using, t-test, although non parametric data."  ))
        }
    }else{
      stop("Please select an apropriate hypothesis testing option. Available options are: 't-test' or 'wilcox-test'")
    }
}
  ...
  #paired end statistics is now enabled
  if (Paired == F){
  T_C1vC2 <-mapply(STAT_pval, x= as.data.frame(Data_DMA_Cond2), y = as.data.frame(Data_DMA_Cond1), SIMPLIFY = F)
  }else{
    message(paste("Paired",STAT_name , "statistics applied"))
    sortedC1 <- Data_DMA_Cond1 %>% arrange(factor(row.names(Data_DMA_Cond1), levels = C1))
    sortedC2 <- Data_DMA_Cond2 %>% arrange(factor(row.names(Data_DMA_Cond2), levels = C2))
    T_C1vC2 <- base::mapply(STAT_pval, x= as.data.frame(sortedC2), y = as.data.frame(sortedC1), paired = T, SIMPLIFY = F)    
  }
  ...
  if(CoRe==TRUE){
    ...
    # Make additional output columns for CoRe both Conditions
    DMA_Output[,paste("CoRe",Condition1,sep = "_")] <- "1"
    for(i in 1:ncol(Data_CoRe_Cond1[1,])){
      DMA_Output[i,5] <- Data_CoRe_Cond1[1,i]
    }
    Mean_value_Cond1 <-  Log2FC_Condition1
    for(i in 1:ncol(Mean_value_Cond1)){
      if(Data_CoRe_Cond1[1,i]=="Consumed"){
        Mean_value_Cond1[,i] = -Mean_value_Cond1[,i]
      }else{
        Mean_value_Cond1[,i] = Mean_value_Cond1[,i]
      }
    }
    DMA_Output[,paste("Mean_value",Condition1,sep = "_")] <- "1"
    for(i in 1:ncol(Mean_value_Cond1[1,])){
      DMA_Output[i,6] <- Mean_value_Cond1[1,i]
    }
    
    DMA_Output[,paste("CoRe",Condition2,sep = "_")] <- "1"
    for(i in 1:ncol(Data_CoRe_Cond2[1,])){
      DMA_Output[i,7] <- Data_CoRe_Cond2[1,i]
    }
    Mean_value_Cond2 <-  Log2FC_Condition2
    for(i in 1:ncol(Mean_value_Cond2)){
      if(Data_CoRe_Cond2[1,i]=="Consumed"){
        Mean_value_Cond2[,i] = -Mean_value_Cond2[,i]
      }else{
        Mean_value_Cond2[,i] = Mean_value_Cond2[,i]
      }
    }
    DMA_Output[,paste("Mean_value",Condition2,sep = "_")] <- "1"
    for(i in 1:ncol(Mean_value_Cond2[1,])){
      DMA_Output[i,8] <- Mean_value_Cond2[1,i]
    }
  }else{
    DMA_Output <- STAT_C1vC2
  }
  #padjVALs = "0.1, 0.05, 0.01"
  #log2FCs = "0.5, 1,2 "
  ...
  DMA_Output_padj_Log2FC <-  DMA_Output %>% dplyr::filter(p.adj < padjVAL & abs(Log2FC) > log2.FC)# %>% order(DMA_Heart$Log2FC)
