...
MetaProVizMCA <- function(Input_data1,
  ...
  if (CoRe==TRUE){
    ...
    # This block checks the input so that C1 in the intracellular and C2 in the extracellular
    if(sum(grepl("CoRe", names(Input_data2)))>0){
      DMA_Intra  <- Input_data1
      DMA_CoRe  <- Input_data2
      DMA_Intra  <- na.omit(DMA_Intra )
      DMA_CoRe  <- na.omit(DMA_CoRe )
    }else{
      DMA_CoRe  <- Input_data2
      DMA_Intra  <- Input_data1
      DMA_CoRe  <- na.omit(DMA_CoRe )
    ...
    Cond1 <- sub("CoRe_", "", names(DMA_CoRe)[which(grepl("CoRe", names(DMA_CoRe)))][1])
    Cond2 <- sub("CoRe_", "", names(DMA_CoRe)[which(grepl("CoRe", names(DMA_CoRe)))][2])
    
    # Add CoRe direction to DMA
    DMA_CoRe <- DMA_CoRe%>%
      mutate(CoRe = case_when(DMA_CoRe[names(DMA_CoRe)[which(grepl("CoRe", names(DMA_CoRe)))][1]] == 'Consumed' & DMA_CoRe[names(DMA_CoRe)[which(grepl("CoRe", names(DMA_CoRe)))][2]] == 'Consumed' ~ 'Consumed',
                              DMA_CoRe[names(DMA_CoRe)[which(grepl("CoRe", names(DMA_CoRe)))][1]] == 'Released' & DMA_CoRe[names(DMA_CoRe)[which(grepl("CoRe", names(DMA_CoRe)))][2]] == 'Released' ~ 'Released',
                              DMA_CoRe[names(DMA_CoRe)[which(grepl("CoRe", names(DMA_CoRe)))][1]] == 'Consumed' & DMA_CoRe[names(DMA_CoRe)[which(grepl("CoRe", names(DMA_CoRe)))][2]] == 'Released' ~ paste('Consumed ',Cond1,' & Released ',Cond2),
                              DMA_CoRe[names(DMA_CoRe)[which(grepl("CoRe", names(DMA_CoRe)))][1]] == 'Released' & DMA_CoRe[names(DMA_CoRe)[which(grepl("CoRe", names(DMA_CoRe)))][2]] == 'Consumed' ~ paste('Released ',Cond1,' & Consumed ',Cond2),
                              TRUE ~ 'No Change'))
    ...
    Merged_DMA_Df <- merge( DMA_Intra, DMA_CoRe, by="Metabolite")
    
    # Define the clusters
    if( test == "p.adj"){
      Merged_DMA_Df <- Merged_DMA_Df%>%
        mutate(`Intracellular_Change` = case_when(Log2FC_Intra >= 0.5 & p.adj_Intra< 0.05 ~ 'UP',
                                                  Log2FC_Intra <= -0.5 & p.adj_Intra< 0.05 ~ 'DOWN',
                                                  TRUE ~ 'No Change'))%>%
        mutate(`CoRe_Change` = case_when(Log2FC_CoRe >= 0.5 & p.adj_CoRe < 0.05 & CoRe =="Released" ~ 'Release UP',
                                         ...
                                         Log2FC_CoRe >= 0.5 & p.adj_CoRe < 0.05 & CoRe == paste('Consumed ',Cond1,' & Released ',Cond2) ~ 'Released/Consumed UP',
                                         Log2FC_CoRe >= 0.5 & p.adj_CoRe < 0.05 & CoRe == paste('Consumed ',Cond1,' & Released ',Cond2) ~ 'Released/Consumed UP',
                                         Log2FC_CoRe <= -0.5 & p.adj_CoRe < 0.05 & CoRe == paste('Released ',Cond1,' & Consumed ',Cond2) ~ 'Released/Consumed DOWN',
                                         Log2FC_CoRe <= -0.5 & p.adj_CoRe < 0.05 & CoRe == paste('Released ',Cond1,' & Consumed ',Cond2) ~ 'Released/Consumed DOWN',
                                         TRUE ~ 'No Change'))
    }else{
      Merged_DMA_Df <- Merged_DMA_Df%>%
        mutate(`Intracellular_Change` = case_when(Log2FC_Intra >= 0.5 & p.val_Intra< 0.05 ~ 'UP',
                                                  Log2FC_Intra <= -0.5 & p.val_Intra< 0.05 ~ 'DOWN',
                                                  TRUE ~ 'No Change'))%>%
        mutate(`CoRe_Change` = case_when(Log2FC_CoRe >= 0.5 & p.val_CoRe < 0.05 & CoRe =="Released" ~ 'Release UP',
                                         ...
                                         Log2FC_CoRe >= 0.5 & p.val_CoRe < 0.05 & CoRe == paste('Consumed ',Cond1,' & Released ',Cond2) ~ 'Released/Consumed UP',
                                         Log2FC_CoRe >= 0.5 & p.val_CoRe < 0.05 & CoRe == paste('Consumed ',Cond1,' & Released ',Cond2) ~ 'Released/Consumed UP',
                                         Log2FC_CoRe <= -0.5 & p.val_CoRe < 0.05 & CoRe == paste('Released ',Cond1,' & Consumed ',Cond2) ~ 'Released/Consumed DOWN',
                                         Log2FC_CoRe <= -0.5 & p.val_CoRe < 0.05 & CoRe == paste('Released ',Cond1,' & Consumed ',Cond2) ~ 'Released/Consumed DOWN',
                                         TRUE ~ 'No Change'))
    }
    ...
    Merged_DMA_Df <- Merged_DMA_Df%>%
      mutate(`Metabolite_Cluster` = case_when(`Intracellular_Change` == "UP" & `CoRe_Change`== "Release UP" ~ '1',
                                              `Intracellular_Change` == "UP" & `CoRe_Change`== "Release DOWN" ~ '2',
                                              `Intracellular_Change` == "UP" & `CoRe_Change`== "Consume UP" ~ '3',
                                              `Intracellular_Change` == "UP" & `CoRe_Change`== "Consume DOWN" ~ '4',
                                              `Intracellular_Change` == "UP" & `CoRe_Change`== "Released/Consumed UP" ~ '5',
                                              `Intracellular_Change` == "UP" & `CoRe_Change`== "Released/Consumed DOWN" ~ '6',
                                              `Intracellular_Change` == "UP" & `CoRe_Change`== "No Change" ~ '7',
                                              `Intracellular_Change` == "No Change" & `CoRe_Change`== "Release UP" ~ '8',
                                              `Intracellular_Change` == "No Change" & `CoRe_Change`== "Release DOWN" ~ '9',
                                              `Intracellular_Change` == "No Change" & `CoRe_Change`== "Consume UP" ~ '10',
                                              `Intracellular_Change` == "No Change" & `CoRe_Change`== "Consume DOWN" ~ '11',
                                              `Intracellular_Change` == "No Change" & `CoRe_Change`== "Released/Consumed UP" ~ '12',
                                              `Intracellular_Change` == "No Change" & `CoRe_Change`== "Released/Consumed DOWN" ~ '13',
                                              `Intracellular_Change` == "No Change" & `CoRe_Change`== "No Change" ~ '14',
                                              `Intracellular_Change` == "DOWN" & `CoRe_Change`== "Release UP" ~ '15',
                                              `Intracellular_Change` == "DOWN" & `CoRe_Change`== "Release DOWN" ~ '16',
                                              `Intracellular_Change` == "DOWN" & `CoRe_Change`== "Consume UP" ~ '17',
                                              `Intracellular_Change` == "DOWN" & `CoRe_Change`== "Consume DOWN" ~ '18',
                                              `Intracellular_Change` == "DOWN" & `CoRe_Change`== "Released/Consumed UP" ~ '19',
                                              `Intracellular_Change` == "DOWN" & `CoRe_Change`== "Released/Consumed DOWN" ~ '20',
                                              `Intracellular_Change` == "DOWN" & `CoRe_Change`== "No Change" ~ '21',
                                              Log2FC_Intra <= -0.5 & p.val_Intra< 0.05 ~ 'DOWN',
                                              TRUE ~ 'X'))
    
    Alluvial_DF2 <-  Merged_DMA_Df
    
    ##Write to file
    writexl::write_xlsx(Alluvial_DF2, paste(results_folder_MCA_folder,"/Metabolic_Clusters_Output_", "padj",pCutoff,"FClog2",FCcutoff,Name, Output_Name,".xlsx", sep = ""))
    
  }else{
    Name = paste("Metabolic_Clusters_Output_",Condition1,"-versus-",Condition2)
    ...
    ##Write to file
    writexl::write_xlsx(Alluvial_DF.final, paste(results_folder_MCA_folder,"/Metabolic_Clusters_Output_", "padj",pCutoff,"FClog2",FCcutoff,Name,Output_Name,".xlsx", sep = ""))
  ...
  for (var in plot_color_remove_variable){
  ...
  Save_as_var(paste(results_folder_MCA_folder,"/AlluvianPlot", "padj",pCutoff,"FClog2",FCcutoff, Name,Output_Name, ".",Save_as, sep=""), width=12, height=9)
...
# Load the data # the f contains metabolites, logFC, pval and padj
#dataNor <- read.csv("data/TotalPools_DMA_KOvWT_DMEM_Normoxia.csv",  row.names = "X")
#dataHyp <- read.csv("data/TotalPools_DMA_KOvWT_DMEM_Hypoxia.csv",  row.names = "X")

# Again Aitors data
#setwd("~/Actual_work/Work_for_Lab/Aitor_ASCA/")
#WGP_1h_vs_Control <- read.csv("~/Actual_work/Work_for_Lab/Aitor_ASCA/Alluvial/WGP_1h_vs_Control.csv")
#WGP_1h_Rot_vs_WGP_1h <- read.csv("~/Actual_work/Work_for_Lab/Aitor_ASCA/Alluvial/WGP_1h_Rot_vs_WGP_1h.csv")
#Alluvial_Plot <- MetabolicClusters(Input_data1=WGP_1h_vs_Control, Input_data2=WGP_1h_Rot_vs_WGP_1h, Condition1="WGP_1h_vs_Control", Condition2="WGP_1h_Rot_vs_WGP_1h",
#                                   pCutoff= 0.05 ,FCcutoff=0.5, test = "p.val")

########################################
### ### ### Run the Analysis ### ### ###
#setwd("~/Actual_work/Metabolomics_Preprocessing")

#Metabolic_Clusters_Output <- MetabolicClusters(Input_data1=dataNor,  Input_data2=dataHyp,
#                                               Condition1="Normoxia",  Condition2="Hypoxia", test = "p.val",
#                                               Output_Name = "Metabolic_Clusters_Output_Hypo-vs-Norm")



#Alluvial_Plot <- MetabolicClusters(Input_data1=DMA_output, Input_data2=DMA_output3,
#                                   Condition1="Rot vs ctlr", Condition2="3NPA vs ctrl",
#                                   pCutoff= 0.05 ,FCcutoff=0.5, test = "p.val",
#                                   plot_column_names= c("Metabolite","class", "MetaboliteChange_Significant", "Overall_Change","Pathway"))
